...
#include "uavobjectgeneratorapi.h"

using namespace std;

bool UAVObjectGeneratorAPI::generate(UAVObjectParser *parser, QString templatepath, QString outputpath)
{
    fieldTypeStrCPP << "char" << "short int" << "int" <<
        "unsigned char" << "unsigned short int" << "unsigned int" << "float" << "unsigned char";

    fieldTypeStrCPPClass << "INT8" << "INT16" << "INT32"
                         << "UINT8" << "UINT16" << "UINT32" << "FLOAT32" << "ENUM";

    apiCodePath        = QDir(templatepath + QString(API_CODE_DIR));
    apiOutputPath      = QDir(outputpath + QString("api"));
    apiOutputPath.mkpath(apiOutputPath.absolutePath());

    apiCodeTemplate    = readFile(apiCodePath.absoluteFilePath("uavobject.cpp.template"));
    apiIncludeTemplate = readFile(apiCodePath.absoluteFilePath("uavobject.h.template"));
    QString apiInitTemplate = readFile(apiCodePath.absoluteFilePath("uavobjectsinit.cpp.template"));

    if (apiCodeTemplate.isEmpty() || apiIncludeTemplate.isEmpty() || apiInitTemplate.isEmpty()) {
        std::cerr << "Problem reading api code templates" << endl;
        return false;
    }
    ...
    for (int objidx = 0; objidx < parser->getNumObjects(); ++objidx) {
        ...
        apiObjInit.append("    objMngr->registerObject( new " + info->name + "() );\n");
        objInc.append("#include \"" + info->namelc + ".h\"\n");
    }

    // Write the api object inialization files
    apiInitTemplate.replace(QString("$(OBJINC)"), objInc);
    apiInitTemplate.replace(QString("$(OBJINIT)"), apiObjInit);
    bool res = writeFileIfDiffrent(apiOutputPath.absolutePath() + "/uavobjectsinit.cpp", apiInitTemplate);
    if (!res) {
...
/**
 * Generate the API object files
 */
bool UAVObjectGeneratorAPI::process_object(ObjectInfo *info)
{
    if (info == NULL) {
        return false;
    }
    ...
    QString outInclude = apiIncludeTemplate;
    QString outCode    = apiCodeTemplate;
    ...
    for (int n = 0; n < info->fields.length(); ++n) {
        ...
        if (field->numElements > 1) {
            propertyGetters +=
                QString("    Q_INVOKABLE %1 get%2(unsigned int index) const;\n")
                .arg(type).arg(field->name);
            propertiesImpl  +=
                QString("%1 %2::get%3(unsigned int index) const\n"
                        "{\n"
            ...
            propertySetters +=
                QString("    void set%1(unsigned int index, %2 value);\n")
                .arg(field->name).arg(type);
            propertiesImpl  +=
                QString("void %1::set%2(unsigned int index, %3 value)\n"
                        "{\n"
                        "   mutex->lock();\n"
            ...
            propertyNotifications +=
                QString("    void %1Changed(unsigned int index, %2 value);\n")
                .arg(field->name).arg(type);
        } else {
    ...
    for (int n = 0; n < info->fields.length(); ++n) {
        ...
        if (info->fields[n]->numElements > 1) {
            ...
            enums.append(QString("    static const unsigned int %1_NUMELEM = %2;\n")
    ...
    if(info->fields.length() > 0){
		// Replace the $(SETDATAFIELD) tag
		body.clear();
		body.append(QString("if(fieldID == \"%1\") {\n").arg(info->fields[0]->name));
		body.append(QString("		memset( &(dataFields.%1), 0, sizeof(dataFields.%1));\n").arg(info->fields[0]->name));
		body.append(QString("		memcpy( &(dataFields.%1), dataIn, sizeof(dataFields.%1));\n").arg(info->fields[0]->name));
		body.append("	}");
		for (int n = 1; n < info->fields.length(); ++n) {
				body.append(QString(" else if(fieldID == \"%1\") {\n").arg(info->fields[n]->name));
				body.append(QString("		memset( &(dataFields.%1), 0, sizeof(dataFields.%1));\n").arg(info->fields[n]->name));
				body.append(QString("		memcpy( &(dataFields.%1), dataIn, sizeof(dataFields.%1));\n").arg(info->fields[n]->name));
				body.append("	}");
		}
		outCode.replace(QString("$(SETDATAFIELD)"), body);

		// Replace the $(GETDATAFIELD) tag
		body.clear();
		body.append(QString("if(fieldID == \"%1\") {\n").arg(info->fields[0]->name));
		body.append(QString("		memcpy( dataOut, &(dataFields.%1), sizeof(dataFields.%1));\n").arg(info->fields[0]->name));
		body.append("	}");
		for (int n = 1; n < info->fields.length(); ++n) {
				body.append(QString(" else if(fieldID == \"%1\") {\n").arg(info->fields[n]->name));
				body.append(QString("		memcpy( dataOut, &(dataFields.%1), sizeof(dataFields.%1));\n").arg(info->fields[n]->name));
		...
		outCode.replace(QString("$(GETDATAFIELD)"), body);

		// Replace the $(PRETTYPRINT) tag
		body.clear();
		body.append(QString("ss << \"\\nPrinting object: %1\" << std::endl;\n").arg(info->namelc));
		for (int n = 0; n < info->fields.length(); ++n) {
				body.append(QString("	ss << \"\\t%1: \" << dataFields.%1 << std::endl;\n").arg(info->fields[n]->name));
		}
		outCode.replace(QString("$(PRETTYPRINT)"), body);
	}
    ...
    // Write the API code
    bool res = writeFileIfDiffrent(apiOutputPath.absolutePath() + "/" + info->namelc + ".cpp", outCode);
    if (!res) {
        cout << "Error: Could not write api output files" << endl;
        return false;
    }
    res = writeFileIfDiffrent(apiOutputPath.absolutePath() + "/" + info->namelc + ".h", outInclude);
    if (!res) {
        cout << "Error: Could not write api output files" << endl;
        return false;
    }
